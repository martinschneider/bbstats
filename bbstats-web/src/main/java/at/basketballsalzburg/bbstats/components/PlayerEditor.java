...
import at.basketballsalzburg.bbstats.utils.CountryValueEncoder;
...
public class PlayerEditor
{
    ...
    @Component(parameters = {"value=playerCountry",
        "model=countrySelectModel", "encoder=countryValueEncoder"})
    ...
    @Inject
    @Property
    private CountryValueEncoder countryValueEncoder;
    ...
    @Property
    @Persist
    private CountryDTO playerCountry;

    void onSuccess()
    {
        player.setNationality(playerCountry.getIsoCode());
    ...
    void setupRender()
    {
        ...
        {
            playerCountry =
                new CountryDTO(player.getNationality().toLowerCase(), countryService.findCountryNameForIsoCode(
                    player.getNationality(),
                    request.getLocale()));
        }
        countrySelectModel = selectModelFactory
            .create(countryService.findAll(request.getLocale()), "countryName");
        ageGroupSelectModel = selectModelFactory.create(
            ageGroupService.findAll(), "name");
        countryValueEncoder.setLocale(request.getLocale());
    }
}
