...
public class MultiSelect2 extends AbstractField {
	...
	StreamResponse onResourceRequested() throws IOException {
		...
		int page = Integer.valueOf(StringUtils.defaultIfBlank(request.getParameter("page"), "1"));
	...
	boolean setupRender() {
		...
		if (trackedValue != null) {
			...
			value = StringUtils.join(provider.toIds(multiValue), ',');
		}
		...
		ajax.setData(String.format("function(term, page) { return { term: term, page: page, ajax: true }; }"));

		ajax.setResults("function(data, page) { return data; }");

		// configure the localized strings/renderers
		settings.setFormatNoMatches("function() { return '" + getEscapedJsString("noMatches") + "';}");
		settings.setFormatInputTooShort("function(input, min) { return min - input.length == 1 ? '" + getEscapedJsString("inputTooShortSingular") + "' : '" + getEscapedJsString("inputTooShortPlural") + "'.replace('{number}', min - input.length); }");
		settings.setFormatSelectionTooBig("function(limit) { return limit == 1 ? '" + getEscapedJsString("selectionTooBigSingular") + "' : '" + getEscapedJsString("selectionTooBigPlural") + "'.replace('{limit}', limit); }");
		settings.setFormatLoadMore("function() { return '" + getEscapedJsString("loadMore") + "';}");
		settings.setFormatSearching("function() { return '" + getEscapedJsString("searching") + "';}");

		String listenerURI = componentResources.createEventLink("resourceRequested").toAbsoluteURI();
	...
	public Collection getConvertedInput(String value) {
		final Collection choices;
		if (StringUtils.isEmpty(value)) {
			choices = new ArrayList();
		} else {
			choices = provider.toChoices(Arrays.asList(value.replaceAll(COMMA_ESCAPING_CHAR, ",").split(",")));
		}
		return choices;
	}
	...
	void afterRender() {
		...
		Collection choices = getConvertedInput(value);
	...
	protected void processSubmission(String controlName)
	{
		...
		try
		{
			Collection translated = getConvertedInput(rawValue);

			putPropertyNameIntoBeanValidationContext("value");

			fieldValidationSupport.validate(translated, componentResources, validate);

			value = rawValue;
			multiValue = translated;
