...
public class GymMaintenance {
	...
	@Component(parameters = { "source=gymsList", "model=gymModel",
			"empty=message:noData", "row=gym", "rowsPerPage=20",
			"include=name,adress,postalCode,city,country", "add=actions,route",
			"reorder=name,adress,postalCode,city,country,route,actions",
	...
	@Component(parameters = { "event=new",
			"Permission.allowedPermissions=newGym" })
	@MixinClasses(Permission.class)
	...
	public Map<String, String> getCssClassMap() {
		...
		values.put("route", CssConstants.HIDDEN_MD_SM_XS);
		values.put("country", CssConstants.HIDDEN_SM_XS);
