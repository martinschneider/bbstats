...
public class Schedule {
	...
	Object onDownloadXLS() throws IOException {
		return new GenericStreamResponse("application/vnd.ms-excel", ".xls",
				xlsGameExporter.getFile(gameService.findAfter(new Date())),
				"spiele");
	}
	...
	Object onDownloadICal() throws IOException {
		return new GenericStreamResponse("text/calendar", ".ical",
				iCalGameExporter.getFile(gameService.findAfter(new Date())),
				"spiele");
	}
	...
	public Map<String, String> getCssClassMap() {
		...
		values.put("gym", CssConstants.HIDDEN_SM_XS);
		values.put("league", CssConstants.HIDDEN_MD_SM_XS);
		return values;
	}
